...
import { fetchAPIAsync, notificationSwal, API_URL_ICONOS, API_URL_ICONOSTYPE, API_URL_ICONOSCOLOR, API_HOST } from 'common/common';
import { columnsIconosList } from 'common/ExportColums';
...
const columns = [
  ...
  { id: 'tipo', label: 'Tipo', minWidth: 50, key: 3 },
  { id: 'color', label: 'Color', minWidth: 10, key: 4 }
];
export default function IconosPersoPage() {
  ...
  useEffect(() => {
    SearchFilter(page);
    fetch(API_URL_ICONOSTYPE)
      .then((response) => response.json())
      .then((data) => setTipo(data))
      .catch((error) => console.error('Error en la solicitud :', error));
    fetch(API_URL_ICONOSCOLOR)
      .then((response) => response.json())
  ...
  async function SearchFilter(numPage) {
    ...
    try {
      const result = await fetchAPIAsync(API_URL_ICONOS, filter, 'GET');
  ...
  async function ExportFilter() {
    ...
    try {
      ...
      if (result && result.data.length > 0) {
        exportToExcel(result.data, columnsIconosList, 'Iconos');
      } else {
  ...
  async function SaveItem() {
    ...
    try {
      ...
      formdata.append('color', itemSave.color);
      ...
      fetch(API_URL_ICONOS, requestOptions)
        .then((response) => response.json())
        .then((result) => {
  ...
  function ValidacionItemSave(itemSave) {
    ...
    if (!itemSave || Object.keys(itemSave).length === 0) {
      ...
      if (!itemSave.url) {
        msgResponse += '* Debe agregar ICONO .<br>';
        response = false;
      }
    }
  ...
  async function UpdateItem() {
    ...
    try {
      ...
      formdata.append('tipo', editedItem.color);
      formdata.append('color', editedItem.tipo);
      ...
      const response = await fetch(API_URL_ICONOS + 'Update/' + elementoId, requestOptions);
  ...
  async function handleEliminar(row) {
    try {
      ...
      const response = await fetch(API_URL_ICONOS + '/' + row.id, requestOptions);
      const result = await response.json();
      notificationSwal('success', result.message);
      handleCloseModal2();
      SearchFilter(page);
  ...
  const handleCloseModal1 = () => {
    setOpenModal1(false);
    //agreguen esto caballeros a sus modulos me canso
    setItemSave({
      nombre: ''
    });
  };
          ...
          <div className="col-sm-9">
            <form className="form row mb-4">
              <div className="col-md-6 col-xl-4">
                <label htmlFor="form_name_icon">NOMBRE:</label>
                <input
                  type="text"
                  ...
                  value={searchFilter.form_name_icon || ''}
                />
              </div>
              <div className="col-md-6 col-xl-4">
                <label htmlFor="form_tipo_icon">TIPO:</label>
                <Autocomplete
                  disablePortal
                  id="combo-box-demo"
                  options={Tipo}
                  onChange={(event, newValue) => {
                    setSearchFilter({ ...searchFilter, form_tipo_icon: newValue ? newValue : '' });
                  }}
                  renderInput={(params) => <TextField {...params} />}
                />
              </div>
              <div className="col-md-6 col-xl-4">
                <label htmlFor="form_color_icon">COLOR:</label>
                <Autocomplete
                  disablePortal
                  id="combo-box-demo"
                  options={Color}
                  onChange={(event, newValue) => {
                    setSearchFilter({ ...searchFilter, form_color_icon: newValue ? newValue : '' });
                  }}
                  renderInput={(params) => <TextField {...params} />}
                />
              </div>
        ...
        <div className="modal-dialog modal-dialog-centered d-flex justify-content-center align-items-center">
          <div className="modal-content">
            <div className="modal-header">
              <h2 className="modal-title">Registre un nuevo ICONO</h2>
                ...
                <div className="row">
                  <div className="col-md-6">
                    <div className="form-group">
                      ...
                      <Autocomplete
                        ...
                        options={Color}
                        onChange={(event, newValue) => {
                          setItemSave({ ...itemSave, color: newValue ? newValue : '' });
                        }}
                  ...
                  <div className="col-md-6">
                    <div className="form-group">
                      ...
                      <input type="file" className="form-control-file" name="url" id="url" onChange={handleSaveChange} />
                    </div>
                  </div>
                </div>
              </form>
              ...
              <form encType="multipart/form-data">
                  ...
                  <div className="col-md-6">
                    <div className="form-group">
                      ...
                      <Autocomplete
                        ...
                        options={Color}
                        onChange={(event, newValue) => {
                          handleEditChange('color', newValue ? newValue : '');
                        }}
                        renderInput={(params) => <TextField {...params} label="Color" />}
                        value={editedItem.color || null}
                      />
                    </div>
                  </div>
                </div>
                <div className="row">
                  <div className="col-md-6">
                    <div className="form-group">
                      <label htmlFor="url">URL(*)</label>
                      <input type="file" className="form-control" name="url" id="url" onChange={handleFileChange} />
                    </div>
                  </div>
                </div>
              </form>
