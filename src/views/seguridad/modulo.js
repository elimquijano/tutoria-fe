...
import { API_URL_MODULO, fetchAPIAsync, notificationSwal, eliminarSwal, editarSwal } from 'common/common';

const columns = [
  { id: 'id', label: 'ID', minWidth: 20, key: 0 },
  { id: 'code', label: 'Codigo', minWidth: 100, key: 1 },
  { id: 'name', label: 'Nombre', minWidth: 100, key: 2 },
  { id: 'description', label: 'Description', minWidth: 170, key: 3 },
  { id: 'route', label: 'Ruta', minWidth: 100, key: 4 }
];
const PageName = 'Modulo';
...
export default function ModuloPage() {
  ...
  async function SearchFilter(numPage) {
    ...
    try {
      const result = await fetchAPIAsync(API_URL_MODULO, filter, 'GET');
  ...
  async function SaveItem() {
    try {
      const result = await fetchAPIAsync(API_URL_MODULO, itemSave, 'POST');
      console.log(result);
  ...
  async function updateItem() {
    ...
    editarSwal(API_URL_MODULO, elementoId, updatedData, SearchFilter);
    handleCloseModal2();
  }
  ...
  function handleEliminar(row) {
    ...
    eliminarSwal(elementoId, API_URL_MODULO, SearchFilter);
  }
        ...
        <div className="row content">
          <div className="col-sm-9">
            <form className="form row mb-4">
              ...
              <div className="col-sm-4">
                <label htmlFor="form_descrip">Descripcion:</label>
                <input
                  type="text"
                  className="form-control"
                  name="form_descrip"
                  id="form_descrip"
                  onChange={handleSearchChange}
                  value={searchFilter.form_descrip || ''}
                />
              </div>
            </form>
          </div>
