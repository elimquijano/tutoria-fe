...
import { URL, API_URL_DETCOMPRA, API_URL_PRODUCTO, fetchAPIAsync, notificationSwal, redirectToRelativePage } from 'common/common';
...
export default function DetCompraPage() {
    ...
    async function SaveItem() {
        try {
            ...
            await fetchAPIAsync(API_URL_DETCOMPRA, dataForm, 'POST');

            notificationSwal('success', '¡Detalles actualizados correctamente!');
            handleCloseModal1();
            SearchFilter(page);
            redirectToRelativePage('/#/admin/compra/');
        } catch (error) {
        ...
        <div>
            <div>
                <div className="form">
                    ...
                    <Button onClick={handleOpenModal1} className='btn btn-info m-3'>Registrar Detalle de compra</Button>
                                            ...
                                            <TableRow hover role="checkbox" tabIndex={-1} key={row.id}>
                                                {columns.map((column) => (
                                                    <TableCell key={column.id} align={column.align}>
                                                        {column.id === 'id_proveedor' ? (
                                                            row.name_proveedor || proveedores.find((prov) => prov.id === row.id_proveedor)?.name
                                                        ) : column.id === 'receipt' ? (
                                                            <div>
                                                                <button className='btn btn-primary' onClick={() => viewReceipt(row[column.id])}>Ver Comprobante</button>
                                                            </div>
                                                        ) : column.id === 'price' ? (
                                            ...
                                            <div className="form-group">
                                                ...
                                                <Autocomplete
                                                    ...
                                                    onChange={(event, newValue) => {
                                                        setItemSave({
                                                            ...itemSave,
                                                            id_producto: newValue ? newValue.id : '',
                                                            name_producto: newValue ? newValue.name : '',
                                                        });
                                                    }}
